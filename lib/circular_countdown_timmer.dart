...
class CountDownTimerState extends State<CountDownTimer>
  ...
  void didUpdateWidget(CountDownTimer oldWidget) {
    ...
    // if (oldWidget.isReverse != widget.isReverse) {
    //   print('CountDownTimerState.didUpdateWidget');
  ...
  void _setController() {
    ...
    widget.controller?._duration = _duration;
    widget.controller?._onDone = widget.onDone;
...
class CountDownController {
  ...
  _watch() {
    if (subs == null) {
      subs = 1;
      _state._controller.addListener(() {
        if (_state._controller.status == AnimationStatus.completed) _onDone();
      });
    }
  }
  ...
  void restart({int duration, bool isReverse, Function onDone}) async {
    _watch();
    _state._controller.duration =
        Duration(seconds: duration ?? _state._controller.duration.inSeconds);
    if (isReverse) {
      _state._controller?.reverse(from: 1)?.whenCompleteOrCancel(onDone);
    } else {
      _state._controller?.forward(from: 0)?.then((_) => onDone);
    }
  }
